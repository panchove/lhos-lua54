...
/* If header failed to define the typedef for any reason, provide a fallback
   local typedef so this compilation unit still builds. The real typedef is in
   components/lhos_post/include/lhos_post.h and will be used when available. */
#ifndef LHOS_POST_H
typedef struct
{
  bool ram_ok;
...
lhos_post_hardware (lhos_post_status_t *status)
{
  ...
  /* Indicate POST in progress */
  lhos_post_led_indicate (NULL); /* sets LED to Blue (in progress) */

  /* NVS */
  ESP_LOGI (TAG, "Running NVS test...");
  g_post_status.nvs_ok = test_nvs ();
  ESP_LOGI (TAG, "NVS test: %s", g_post_status.nvs_ok ? "PASS" : "FAIL");
  if (g_post_status.nvs_ok)
    {
      ESP_LOGI (TAG, "LED: setting GREEN for NVS PASS");
      lhos_ws2812b_set_color_enum (2); /* GREEN */
      ESP_LOGI (TAG, "LED: GREEN set for NVS PASS");
      vTaskDelay (pdMS_TO_TICKS (200));
    }
  else
    {
      ESP_LOGI (TAG, "LED: flashing 1 for NVS FAIL");
      lhos_ws2812b_flash (1, 150, 150);
      ESP_LOGI (TAG, "LED: flashed 1 for NVS FAIL");
      vTaskDelay (pdMS_TO_TICKS (300));
    }

  ESP_LOGI (TAG, "Running RAM test...");
  g_post_status.ram_ok = test_ram ();
  ESP_LOGI (TAG, "RAM test: %s", g_post_status.ram_ok ? "PASS" : "FAIL");
  if (g_post_status.ram_ok)
    {
      ESP_LOGI (TAG, "LED: setting GREEN for RAM PASS");
      lhos_ws2812b_set_color_enum (2); /* GREEN */
      ESP_LOGI (TAG, "LED: GREEN set for RAM PASS");
      vTaskDelay (pdMS_TO_TICKS (200));
    }
  else
    {
      ESP_LOGI (TAG, "LED: flashing 2 for RAM FAIL");
      lhos_ws2812b_flash (2, 150, 150);
      ESP_LOGI (TAG, "LED: flashed 2 for RAM FAIL");
      vTaskDelay (pdMS_TO_TICKS (300));
    }
  ...
  if (g_post_status.flash_ok)
    {
      ESP_LOGI (TAG, "LED: setting GREEN for Flash PASS");
      lhos_ws2812b_set_color_enum (2); /* GREEN */
      ESP_LOGI (TAG, "LED: GREEN set for Flash PASS");
      vTaskDelay (pdMS_TO_TICKS (200));
    }
  else
    {
      ESP_LOGI (TAG, "LED: flashing 3 for Flash FAIL");
      lhos_ws2812b_flash (3, 150, 150);
      ESP_LOGI (TAG, "LED: flashed 3 for Flash FAIL");
      vTaskDelay (pdMS_TO_TICKS (300));
    }

  ESP_LOGI (TAG, "Running GPIO test...");
  g_post_status.gpio_ok = test_gpio ();
  ESP_LOGI (TAG, "GPIO test: %s", g_post_status.gpio_ok ? "PASS" : "FAIL");
  if (g_post_status.gpio_ok)
    {
      ESP_LOGI (TAG, "LED: setting GREEN for GPIO PASS");
      lhos_ws2812b_set_color_enum (2); /* GREEN */
      ESP_LOGI (TAG, "LED: GREEN set for GPIO PASS");
      vTaskDelay (pdMS_TO_TICKS (200));
    }
  else
    {
      ESP_LOGI (TAG, "LED: flashing 4 for GPIO FAIL");
      lhos_ws2812b_flash (4, 150, 150);
      ESP_LOGI (TAG, "LED: flashed 4 for GPIO FAIL");
      vTaskDelay (pdMS_TO_TICKS (300));
    }

  ESP_LOGI (TAG, "Running PSRAM test...");
  g_post_status.psram_ok = test_psram ();
  ESP_LOGI (TAG, "PSRAM test: %s", g_post_status.psram_ok ? "PASS" : "FAIL");
  if (g_post_status.psram_ok)
    {
      ESP_LOGI (TAG, "LED: setting GREEN for PSRAM PASS");
      lhos_ws2812b_set_color_enum (2); /* GREEN */
      ESP_LOGI (TAG, "LED: GREEN set for PSRAM PASS");
      vTaskDelay (pdMS_TO_TICKS (200));
    }
  else
    {
      ESP_LOGI (TAG, "LED: flashing 5 for PSRAM FAIL");
      lhos_ws2812b_flash (5, 150, 150);
      ESP_LOGI (TAG, "LED: flashed 5 for PSRAM FAIL");
      vTaskDelay (pdMS_TO_TICKS (300));
    }
  ...
  if (g_post_status.wifi_ok)
    {
      ESP_LOGI (TAG, "LED: setting GREEN for WiFi PASS");
      lhos_ws2812b_set_color_enum (2); /* GREEN */
      ESP_LOGI (TAG, "LED: GREEN set for WiFi PASS");
      vTaskDelay (pdMS_TO_TICKS (200));
    }
  else
    {
      ESP_LOGI (TAG, "LED: flashing 6 for WiFi FAIL");
      lhos_ws2812b_flash (6, 150, 150);
      ESP_LOGI (TAG, "LED: flashed 6 for WiFi FAIL");
      vTaskDelay (pdMS_TO_TICKS (300));
    }

  ESP_LOGI (TAG, "Running BLE test...");
  g_post_status.ble_ok = test_ble ();
  ESP_LOGI (TAG, "BLE test: %s", g_post_status.ble_ok ? "PASS" : "FAIL");
  if (g_post_status.ble_ok)
    {
      ESP_LOGI (TAG, "LED: setting GREEN for BLE PASS");
      lhos_ws2812b_set_color_enum (2); /* GREEN */
      ESP_LOGI (TAG, "LED: GREEN set for BLE PASS");
      vTaskDelay (pdMS_TO_TICKS (200));
    }
  else
    {
      ESP_LOGI (TAG, "LED: flashing 7 for BLE FAIL");
      lhos_ws2812b_flash (7, 150, 150);
      ESP_LOGI (TAG, "LED: flashed 7 for BLE FAIL");
      vTaskDelay (pdMS_TO_TICKS (300));
    }
